...
#
# 使い方:
#   bash scripts/update-ci-auth-server-url.sh <api-gateway-id>
#
...
NEW_URL="https://ample-precise-knee.ngrok-free.dev/restapis/${API_GATEWAY_ID}/local/_user_request_"
...
for file in "${WORKFLOWS_DIR}"/*-ci.yml; do
  sed -i "s|AUTH_SERVER_URL:.*|AUTH_SERVER_URL: ${NEW_URL}|g" "${file}"
